...
- ` ```landscapebreak ``` ` : saut de page en mode Poster (le bouton « Saut de page (Poster) » du menu +). Le Poster est composé dans le Markdown : `landscapebreak` commence une page A4 paysage, `columnbreak` (« Saut de colonne (Poster) ») une colonne dans la page en cours, et chaque colonne est réduite pour tenir dans la hauteur de la page ; sans marqueur, tout le cours est une seule page d'une seule colonne. Quand une page reste trop chargée même réduite au maximum, une bulle le signale et invite à ajouter un saut. Les marqueurs Poster sont sans effet en mode Livre.
