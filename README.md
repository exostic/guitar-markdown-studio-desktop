...
- ` ```landscapebreak ``` ` : saut de page en mode Poster (le bouton « Saut de page (Poster) » du menu +). Sans `columnbreak` dans le document, le mode Poster met chaque page en page de lui-même : une page est tout ce qui se trouve entre deux `landscapebreak` (un cours sans marqueur tient sur une seule page), ses blocs (titre, paragraphe, tablature, grille…) restent entiers et sont répartis dans l'ordre sur trois colonnes aussi pleines les unes que les autres (`poster-columns: 2`, de 1 à 4, dans le front matter change ce nombre), un titre n'est jamais seul en bas de colonne, puis les colonnes sont réduites ensemble pour tenir dans la page A4 paysage : trop de contenu entre deux sauts donne une page en petits caractères, ajoutez un saut. Dès qu'un `columnbreak` est présent, la mise en page est entièrement manuelle : `landscapebreak` commence une page, `columnbreak` une colonne, chaque colonne est réduite pour tenir. Le mode Livre n'est pas concerné.
